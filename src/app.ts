...
import { connectDB, sequelize } from './config/database'; // Import connector
...
const start = async () => {
    try {
        ...
        // --- SEED ORM DATA (Optional for Demo) ---
        ...
        if (userCount === 0) {
            ...
            for (let i = 1; i <= 10; i++) {
                await Post.create({ content_url: `https://v/${i}.mp4`, category: 'fun' });
            }
        }
        // -----------------------------------------
